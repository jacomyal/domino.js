...
a.id+'">'+(a.label||a.id)+"</option>"})).val(c))}i.module.call(this);var a=this,b=h||{};if(!b.property)throw Error("[Select] Property missing");var d=b.dispatch||(1===f.getEvents(b.property).length?f.getEvents(b.property)[0]:null),c,j,k=b.element||$("<select></select>");b.cssClass&&k.addClass(b.cssClass);b.cssId&&k.attr("id",b.cssId);b.values&&"array"===i.struct.get(b.values)&&(j=b.values,k.append(j.map(function(a){return"string"===typeof a?'<option value="'+a+'">'+a+"</option>":'<option value="'+
