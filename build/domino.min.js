...
g)?x('Property "'+b+'": Wrong type error'):T[b]=g;return!0};a.getter!==f&&("function"!==p.get(a.getter)?x('Property "'+b+'": Getter is not a function'):(J[b]=a.getter,pa[b]=!0));J[b]=J[b]||function(){return T[b]};if(a.value!==f||U[b])a.value!==f?ba(b,a.value):G('Property "'+b+'": Initial value is missing');if(a.triggers!==f)for(l in!p.check("array|string",a.triggers)&&x('Property "'+b+'": Events ("triggers") must be specified in an array or separated by spaces in a string'),V[b]=y.array(a.triggers),
V[b]||[])W[V[b][l]]=W[V[b][l]]||[],W[V[b][l]].push(b);a.dispatch!==f&&(p.check("array|string",a.dispatch)?ca[b]=y.array(a.dispatch):x('Property "'+b+'": Events ("dispatch") must be specified in an array or separated by spaces in a string'));return C}function v(b){var g=b||{};g.id!==f&&"string"===p.get(g.id)||u("The service id is not indicated.");p.check("function|string",g.url)||u("The service URL is not valid.");da[g.id]!==f&&u('The service "'+g.id+'" already exists.');da[g.id]=function(b){G('Calling service "'+
...
p.get(h.data[c])&&h.data[c].match(e)&&(h.data[c]=X(h.data[c].match(e)[1],d),k=!0);h.success=function(b){var l,k,w,n,q={},qa=[],u=[],v={},A=!1,t=a.path||g.path,u=a.setter||g.setter,ba=a.success||g.success;n=a.expect||g.expect||B("expect");if("function"!==p.get(n)||n.call(m(),b,a,g)){G('Service "'+g.id+'" successfull.');"string"===p.get(u)&&u.match(e)&&(u=X(u.match(e)[1],d));"string"===p.get(t)&&t.match(e)&&(t=X(t.match(e)[1],d));n=b;(t||"").match(/^(?:\w+\.)*\w+$/)?k=p.get(t,"string")?t.split("."):
f:"string"===p.get(t)&&x('Path "'+t+'" does not match RegExp /^(?:\\w+\\.)*\\w+$/');if(k)for(l in k)n=n[k[l]],n===f&&x('Wrong path "'+t+'" for service "'+g.id+'".');k=y.array(g.events);for(l in k)q[k[l]]=1,A=!0;k=y.array(a.events);for(l in k)q[k[l]]=1,A=!0;u&&I[u]&&n!==f&&(v[u]=n,A=!0);if("function"===p.get(ba)){b=D(ba,{parameters:[b,a],scope:{request:!0,dispatchEvent:!0,update:!0}});k=y.array(b.events);for(c in k)A=!0,q[k[c].type]=1;for(c in b.update)v[c]===f?(v[c]=b.update[c],A=!0):x('The key "'+
c+'" is nor a method neither a property.');(b.services||[]).length&&(A=!0,qa=qa.concat(b.services));s(b)}u=[];for(w in q)u.push(C.getEvent(w));A&&(a.loop||N)({events:u,update:v,services:qa})}else G('"expect" test failed for service "'+g.id+'".'),h.error.call(this,"Unexpected data received.",this.xhr())};k=a.before||g.before;null!=k&&"function"===p.get(k)&&D(k,{parameters:[a],loop:!0,scope:{dispatchEvent:!0,update:!0}});a.abort&&ia[g.id]&&ia[g.id].abort();ia[g.id]=y.ajax(h)};return C}function e(b,
g,a){var r;a={};var d={},h,k;b===f&&u("Module class not specified.");"function"!==p.get(b)&&u("First parameter must be a function.");b.apply(a,(g||[]).concat(m()));b=a.triggers||{};for(k in b.events||{})O[k]=O[k]||[],O[k].push(b.events[k]);for(h in b.properties||{}){for(r in ca[h]||[])P[h]=P[h]||[],P[h].push(b.properties[h]);J[h]!==f&&(A(h),D(b.properties[h],{parameters:[m()]}))}for(k in W||{})d[k]=1;for(k in K||{})d[k]=1;for(k in L||{})d[k]=1;for(k in Q||{})d[k]=1;for(k in d)a.addEventListener(k,
...
1;for(h in Q||{})f[h]=1;for(h in f)b.removeEventListener(h,w);R.splice(R.indexOf(b),1);return b}function c(b,g){var a="object"===typeof b&&1===arguments.length?b||{}:{};"string"===typeof b&&(a[b]=g);N({update:a});return this}function w(b){N({events:[b],emitter:b.target})}function N(b){var a,l,r,d,h,k,c,w=!1,e=[],n=[],q=[];b=b||{};var p={},t={};b.loop=(+b.loop||0)+1;b.loopId=b.loopId||++ka;B("maxDepth")&&b.loop>B("maxDepth")&&u("Loop "+b.loopId+" exceeds maximum depth ("+B("maxDepth")+")");var A=y.array(b.events);
c=y.array(b.services);var v=b.update||{};if(B("verbose")){G("Iteration "+b.loop+" (loop "+b.loopId+")");if(A.length){h=[];for(l in A)h.push(A[l].type);G(" -> Events: ",h)}if(c.length){h=[];for(l in c)h.push("string"===typeof c[l]?c[l]:c[l].service);G(" -> Services: ",h)}h=[];for(l in v)h.push(l);h.length&&G(" -> Update: ",h)}for(a in v)if(I[a]===f)x('The property "'+a+'" is not referenced.');else if(h=ba(a,v[a])||!!b.force){for(l in P[a])D(P[a][l],{parameters:[m(),{property:a,emitter:b.emitter}]});
for(l in ca[a]||[])h=ca[a][l],p[h]=C.getEvent(h)}if(c.length)if(B("mergeRequests"))Y(c);else for(r in c)Y(c[r]);for(l in A){k=A[l];for(r in c=k.data||{},a=W[k.type]||[],a)c[a[r]]!==f&&(w=!0,t[a[r]]=c[a[r]]);for(d in O[k.type])D(O[k.type][d],{parameters:[m(),k,b.emitter]});for(r in K[k.type]||[])if(0>e.indexOf(K[k.type][r])){e.push(K[k.type][r]);c=D(K[k.type][r],{parameters:[k],scope:{request:!0,dispatchEvent:!0,update:!0}});a=y.array(c.events);for(d in a)p[a[d].type]=a[d];for(d in c.update)t[d]===
f?(w=!0,t[d]=c.update[d]):x('The property "'+d+'" has already been updated in the current loop.');(c.services||[]).length&&(w=!0,q=q.concat(c.services));s(c)}for(r in L[k.type]||[])h=L[k.type][r],p[h]=C.getEvent(h);if(B("logDescriptions"))for(r in Q[k.type]||[])G("[HACK]",Q[k.type][r])}for(k in p)C.dispatchEvent(k,p[k].data),n.push(p[k]),w=!0;w&&N({events:n,update:t,services:q,emitter:b.emitter,loopId:b.loopId,loop:b.loop})}function A(b){if(J[b]){if(pa[b]){for(var a=[],l={},d=1,c=arguments.length;d<
c;d++)a.push(arguments[d]);l[b]=T[b];a=D(J[b],{parameters:a,inputValues:l});return M(b)?y.clone(a.returned):a.returned}return M(b)?y.clone(J[b]()):J[b]()}x('Property "'+b+'" not referenced.')}function ba(b,a){if(I[b]){if(oa[b]){var l,d;l=[];d={};M(b)&&(a=y.clone(a));d[b]=A(b);for(var c=1,h=arguments.length;c<h;c++)l.push(arguments[c]);d=D(I[b],{parameters:l,inputValues:d});(l="boolean"!==p.get(d.returned)||d.returned)&&(T[b]=d.update[b]);return l}return I[b].call(m(),M(b)?y.clone(a):a)}x('Property "'+
...
typeof a&&(m[a]=q);for(var s in m)m[s]!==f?S[s]=m[s]:delete S[s];return this};E.instances=function(a){if(arguments.length)return aa[a];F("[domino.global] You need to indicate a name to get the instance.")};E.EventDispatcher=function(){Object.defineProperty(this,"_handlers",{value:{}})};var z=E.EventDispatcher;z.prototype.addEventListener=function(a,f){if(arguments.length)if(1===arguments.length&&"object"===M.type.get(arguments[0]))for(a in arguments[0])this.addEventListener(a,arguments[0][a]);else if(1<
arguments.length){var m;a=arguments[0];f=arguments[1];var s=M.array(a),n;for(n in s)m=s[n],this._handlers[m]||(this._handlers[m]=[]),this._handlers[m].push({handler:f})}return this};z.prototype.removeEventListener=function(a,f){var m,s,n,v,e=M.array(a);if(!arguments.length){for(m in this._handlers)delete this._handlers[m];return this}if(f)for(m in e){v=e[m];if(this._handlers[v]){n=[];for(s in this._handlers[v])this._handlers[v][s].handler!==f&&n.push(this._handlers[v][s]);this._handlers[v]=n}this._handlers[v]&&
0===this._handlers[v].length&&delete this._handlers[v]}else for(m in e)delete this._handlers[e[m]];return this};z.prototype.dispatchEvent=function(a,q){var m,s,n,v,e,d=M.array(a);q=q===f?{}:q;for(m in d)if(e=d[m],this._handlers[e]){v=this.getEvent(e,q);n=[];for(s in this._handlers[e])this._handlers[e][s].handler(v),this._handlers[e][s].one||n.push(this._handlers[e][s]);this._handlers[e]=n}return this};z.prototype.getEvent=function(a,f){return{type:a,data:f||{},target:this}};E.module=function(){z.call(this);
for(var a in z.prototype)this[a]=z.prototype[a];this.triggers={properties:{},events:{}}}})(window);
